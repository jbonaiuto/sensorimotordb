...
from mirrordb.models import Experiment, Unit, BrainRegion, RecordingTrial, Event, GraspObservationCondition, Species, GraspPerformanceCondition, Condition, UnitRecording
...
def remove_all(db='default'):
    ...
    cursor.execute('DELETE FROM %s.mirrordb_graspperformancecondition WHERE 1=1' % (settings.DATABASES[db]['NAME']))
    cursor.execute('DELETE FROM %s.mirrordb_graspcondition WHERE 1=1' % (settings.DATABASES[db]['NAME']))
    cursor.execute('DELETE FROM %s.mirrordb_condition WHERE 1=1' % (settings.DATABASES[db]['NAME']))
    for exp in Experiment.objects.using(db).all():
        cursor.execute('DELETE FROM %s.mirrordb_experiment WHERE id=%d' % (settings.DATABASES[db]['NAME'],exp.id))        
    cursor.close()

def import_kraskov_data(mat_file, video_dir, db='default'):
    exps={}

    mirrordb_video_dir=os.path.join(settings.MEDIA_ROOT,'video')
    ...
    for exp_id, exp in exps.iteritems():
        ...
        copyfile(os.path.join(video_dir,'ring.mp4'),os.path.join(mirrordb_video_dir,'condition_%d.mp4' % exp_conditions[exp_id]['mov_ring'].id))
        ...
        copyfile(os.path.join(video_dir,'sphere.mp4'),os.path.join(mirrordb_video_dir,'condition_%d.mp4' % exp_conditions[exp_id]['mov_sphere'].id))
        ...
        copyfile(os.path.join(video_dir,'trapezoid.mp4'),os.path.join(mirrordb_video_dir,'condition_%d.mp4' % exp_conditions[exp_id]['mov_trapezoid'].id))
